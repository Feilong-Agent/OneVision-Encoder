#!/bin/bash
# Example script for running semantic specificity experiment
# This replaces motion patches with motion patches from unrelated videos
...
echo "K_keep: ${K_KEEP}"
echo "Batch size: ${BATCH_SIZE} (must be >= 2 for cross-video replacement)"
echo ""

# Run with semantic specificity intervention (replace motion with unrelated motion)
python eval_encoder/attentive_probe_codec.py \
    ...
    --save_report "fewshot_video_report/ActionRecognition_semantic_specificity"

echo ""
echo "Experiment completed!"
echo "Results saved to: fewshot_video_report/ActionRecognition_semantic_specificity"
echo ""
