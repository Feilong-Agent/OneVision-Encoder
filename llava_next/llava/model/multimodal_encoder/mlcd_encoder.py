...
class MLCDVisionTower(nn.Module):
    ...
    def feature_select(self, image_forward_outs):
        ...
        elif select_feature_type == "patch_interpolate":
            image_features = image_features[:, 1:]
            # Add bilinear interpolation here
            B, N, C = image_features.shape
            H = W = int(N**0.5)
            image_features = image_features.reshape(B, H, W, C).permute(0, 3, 1, 2)  # B, H, W, C -> B, C, H, W
            image_features = torch.nn.functional.interpolate(image_features, size=(H // 2, W // 2), mode='bilinear', align_corners=False)  # B, C, H, W -> B, C, H/2, W/2
            image_features = image_features.permute(0, 2, 3, 1).reshape(B, (H // 2) * (W // 2), C)  # B, C, H/2, W/2 -> B, H/2, W/2, C -> B, H/2*W/2, C
    ...
    def num_patches_per_side(self):
        if self.select_feature == "patch_interpolate" or self.projector_type == "patch_merger":
    ...
    def num_patches(self):
        if self.select_feature == "patch_interpolate" or self.projector_type == "patch_merger":
            _num_patches = (self.config.image_size // (self.config.patch_size * 2)) ** 2
        else:
