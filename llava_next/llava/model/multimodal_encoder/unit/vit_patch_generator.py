...
class ViTPatchGenerator(nn.Module):
    ...
    def embed_patches(self, x: torch.Tensor) -> torch.Tensor:
        ...
        dtype = patches.dtype
        # patches = self.embedder(patches.to(torch.float32)).to(dtype)
    ...
    def _get_pos_embeddings(self, batch_size: int, input_dims: Tuple[int, int]):
        ...
        # pos_embed = pos_embed.to(dtype=torch.float32)
        # pos_embed = pos_embed.to(dtype=torch.float32)
        ...
        if self.cpe_mode:
            if self.training:
                ...
                # Ensure all intermediate tensors use the same dtype as pos_embed
                scale = torch.rand(batch_size, 1, 1, device=pos_embed.device, dtype=dtype) * (1 - min_scale) + min_scale
                aspect_min = math.log(3 / 4)
                aspect_max = -aspect_min
                aspect = torch.exp(torch.rand(batch_size, 1, 1, device=pos_embed.device, dtype=dtype) * (aspect_max - aspect_min) + aspect_min)

                scale_x = scale * aspect
                scale_y = scale * (1 / aspect)
                scale_xy = torch.stack([scale_x, scale_y], dim=-1).clamp_(0, 1)

                pos_xy = torch.rand(batch_size, 1, 1, 2, device=pos_embed.device, dtype=dtype) * (1 - scale_xy)

                lin_x = torch.linspace(0, 1, steps=input_dims[1], device=pos_embed.device, dtype=dtype)[None, None].expand(batch_size, input_dims[0], -1)
                lin_y = torch.linspace(0, 1, steps=input_dims[0], device=pos_embed.device, dtype=dtype)[None, :, None].expand(batch_size, -1, input_dims[1])
                ...
                # Make sure grid_xy has the same dtype as pos_embed
                grid_xy = grid_xy.to(dtype=dtype)

                # Use grid_sample with consistent dtypes

                dtype = pos_embed.dtype
                # 修改后的代码
                pos_embed = F.grid_sample(
                    pos_embed.expand(batch_size, -1, -1, -1).to(dtype=torch.float32),
                    grid=grid_xy.to(dtype=torch.float32),
                    mode='bilinear',
                    padding_mode='zeros',
                    align_corners=True,
                )
                ...
                pos_embed = F.interpolate(pos_embed, size=(max_dim, max_dim), align_corners=True, mode='bilinear').to(pos_embed.dtype)
