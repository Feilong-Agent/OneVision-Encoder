_base_ = [
    "../_base_/datasets/thumos-14/features_univit.py",  # dataset config
    "../_base_/models/temporalmaxer.py",  # model config
]

model = dict(
    projection=dict(in_channels=384, drop_out=0.2),
    neck=dict(norm_cfg=dict(type="GN", num_groups=4, affine=False)),
)

solver = dict(
    train=dict(batch_size=4, num_workers=2),
    val=dict(batch_size=4, num_workers=1),
    test=dict(batch_size=4, num_workers=1),
    clip_grad_norm=1,
    ema=True,
)
...
work_dir = "exps/thumos/temporalmaxer_univit"
