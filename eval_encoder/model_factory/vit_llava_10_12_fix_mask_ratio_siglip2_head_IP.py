...
class LlavaViTEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, ip_mask = None, mask_ratio=0.5):
        ...
        if t_frames == 1 or self.mask_ratio == 0.0:
            ...
            topk_keep = self.keep_frame * patches_per_frame
            visible_indices, visible_mask_bool, ids_restore = self.mask_by_residual_topk(
                res=ip_mask,                # (B,1,T,H,W)；I 帧建议预先置 0
                k_keep=topk_keep      # 你的超参，比如 int( (1-mask_ratio)*total_patches )
            )
        else:
            # masking
...
@register_model
def pretrain_decoder_small_patch16_224_v10_12_rms_ip(pretrained: bool = False, **kwargs):
