import torch
from model_factory.layers_mlcd import TransformerCausal, VisionRotaryEmbedding, Siglip2MultiheadAttentionPoolingHead
...
class VisualTransformer(nn.Module):
    ...
    def tem_rot_pos_emb(self, grid_thw, half_head_dim=None):
        t, h, w = grid_thw
        ...
        temperal_pos_emb = rotary_pos_emb_full[tpos_ids][:, :half_head_dim//4]
        spatial_h_pos_emb = rotary_pos_emb_full[pos_h_ids][:, :, half_head_dim//4:half_head_dim//8*5].flatten(0, 1)
        spatial_w_pos_emb = rotary_pos_emb_full[pos_w_ids][:, :, half_head_dim//8*5:].flatten(0, 1)
    ...
    def forward(self, x: torch.Tensor, mask_ratio=0.0):
        ...
        rotary_pos_emb = self.tem_rot_pos_emb(torch.tensor(patch_grid, device=x.device),  self.half_head_dim)
        ...
        if self.use_head:
            head_output = self.head(x)  # (B, hidden_size)
...
@register_model
def UniViT_S_16_384_casual_unmask(pretrained: bool = False, **kwargs):
