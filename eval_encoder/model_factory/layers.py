...
class VisionRotaryEmbedding(nn.Module):
    def __init__(self, dim, theta=1000.0):
        super().__init__()
        inv_freq = 1.0 / (theta ** (torch.arange(0, dim, 2, dtype=torch.float) / dim))
        self.register_buffer("inv_freq", inv_freq, persistent=False)
...
class VisionRotaryEmbeddingNorm(nn.Module):
    """
    归一化版 Vision RoPE: 适配多分辨率视频输入
    - 空间坐标 (h, w) 归一化到 [-1, 1]
    - 时间坐标 (t) 归一化到 [-1, 1]
    - 不同大小帧 (14x14 vs 7x7) 自动归一化，避免频率错位
    """

    def __init__(self, dim, theta=100.0):
        super().__init__()
        # base frequency表 (等比递减)
        inv_freq = 1.0 / (theta ** (torch.arange(0, dim, 2, dtype=torch.float) / dim))
        self.register_buffer("inv_freq", inv_freq, persistent=False)

    def forward(self, seqlen):
        """构造标准RoPE表 [max_len, D]"""
        seq = torch.linspace(-1.0, 1.0, steps=seqlen, device=self.inv_freq.device, dtype=self.inv_freq.dtype)
        # 相位矩阵 = 外积
        return 2 * torch.pi * torch.outer(seq, self.inv_freq)  # [L, D/2]
...
class VisionSdpaAttentionCausal(nn.Module):
    ...
    def forward(self, hidden_states, rotary_pos_emb=None, attention_mask=None):
        ...
        if rotary_pos_emb is not None:
            ...
            cos = rotary_pos_emb.cos()
            sin = rotary_pos_emb.sin()
            cos = torch.cat([cos, cos], dim=-1).unsqueeze(2).float()  # (B,L,1,D)
            ...
            q = (q * cos) + (rotate_half_local(q) * sin).to(orig_dtype)
            k = (k * cos) + (rotate_half_local(k) * sin).to(orig_dtype)
...
class PatchMerger(nn.Module):
    """
    将 4 个相邻 patch (2x2) 合并成一个大 patch。
    输入形状: (B, C, T2, H, W)，其中 T2=4
    输出形状: (B, C_out, H_out, W_out)
    """
    def __init__(self, embed_dim):
        super().__init__()
        # 这里 conv1 相当于 patch_attn，用 2x2 stride=2 做融合
        self.conv1 = nn.Conv2d(embed_dim, embed_dim, kernel_size=2, stride=2)

    def forward(self, x_non):
        # x_non: (B, C, T2, H, W)
        b, c, t2, h, w = x_non.shape

        # 假设 T2 = 4，reshape 成 (B, C, 2, 2, H, W)
        side = int(t2 ** 0.5)
        x_non = x_non.reshape(b, c, side, side, h, w)

        # 把 patch 排列在空间上拼接成一张更大的 feature map
        x_non = x_non.permute(0, 1, 4, 2, 5, 3)  # (B, C, H, side, W, side)
        x_non = x_non.reshape(b, c, h * side, w * side)  # 拼出更大的空间 (B, C, H*2, W*2)

        # 对拼好的大patch应用卷积聚合 (2x2 stride=2)
        x_non = self.conv1(x_non)  # -> (B, C, H, W)

        # 如果你还想保持原始时间维度 (T2)，可以再 reshape 回去
        # 但通常在合并后就只剩一个尺度
        return x_non
