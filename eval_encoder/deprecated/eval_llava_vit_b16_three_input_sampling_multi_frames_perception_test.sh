...
targets=(196 392 588 784 980 1176 1372 1568)
...
for i in "${!targets[@]}"; do
  ...
  port=$((BASE_PORT + i))
  gpu=$((GPU_START + i))
  output="output/three_input_b16_sampling_multi_frames_${target}"
  ...
  DATASETS="${DATASETS}" \
  CKPT_PATH="${CKPT_PATH}" \
  bash video_attentive_probe.sh > "${output}/${DATASETS}_test.log" &
