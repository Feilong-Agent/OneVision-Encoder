_base_ = [
    "../_base_/datasets/thumos-14/feature_videomae_small.py",  # dataset config
    "../_base_/models/actionformer.py",  # model config
]

model = dict(projection=dict(in_channels=384, input_pdrop=0.2))

solver = dict(
    train=dict(batch_size=8, num_workers=2),
    val=dict(batch_size=8, num_workers=1),
    test=dict(batch_size=8, num_workers=1),
    clip_grad_norm=1,
    ema=True,
)
...
work_dir = "exps/thumos/actionformer_videomae_small"
