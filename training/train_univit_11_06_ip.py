...
parser.add_argument("--target_num", type=int, default=1536, help="Sampled indices count / 采样索引个数")
    ...
    while not end_of_batch:
        ...
        for head_id, dataset_config in enumerate(args.list_datasets):
            ...
            if dataset_config.dali_type in ["decord"]:
                ...
                visible_indices = list_data_batch[head_id]["visible_indices"]  # [bs, 2000]
                bs = visible_indices.shape[0]
                # assert visible_indices.shape[1] == args.total_indices

                must_have = visible_indices[:, :args.must_num]  # [bs, 196]
                candidates = visible_indices[:, args.must_num:args.total_indices]  # [bs, 1804]
                sample_num = args.target_num - args.must_num  # 1340

                # [bs, 1804]——每行是每个batch的随机分配概率
                randmat = torch.rand(bs, candidates.shape[1], device=visible_indices.device)
                # 用 topk 按行取采样下标
                idx = randmat.topk(sample_num, dim=1).indices  # [bs, 1340]

                # 用 gather 实现批量采样
                sampled = torch.gather(candidates, 1, idx)  # [bs, 1340]

                # 拼接
                selected_visible_indices = torch.cat([must_have, sampled], dim=1)  # [bs, 1536]

                list_batch_sizes.append(head_input.size(0))
                with torch.amp.autocast(dtype=torch.bfloat16, device_type="cuda"):
                    head_embedding = backbone(head_input, selected_visible_indices)["head_output"]
