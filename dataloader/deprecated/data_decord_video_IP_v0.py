...
class DALIWarper(object):
    ...
    def __next__(self):
        try:
            ...
            teacher_videos = data_dict["teacher_videos"]
            I_lists = data_dict["I_lists"]
            P_lists = data_dict["P_lists"]
            return videos, teacher_videos, I_lists, P_lists
...
class ExternalInputCallable:
    ...
    def sparse_sampling_get_frameid_data(
        ...
        if self.mode in ["train"]:
            ...
            i_index = decord_vr.get_key_indices()
            # print("initial", i_index)
            i_index = list(map(int, i_index))
            i_set = set(int(i) for i in i_index)
            ...
            for pos, frame in enumerate(frame_id_list):
                if frame in i_index:
                    I_list.append(pos)
                elif frame in p_index:
                    P_list.append(pos)

                decord_vr.seek(0)
                video_data = decord_vr.get_batch(frame_id_list).asnumpy()
            
            # print("I_list, P_list: ", I_list, P_list)
            I_list = np.array(I_list, dtype=np.int64)
            P_list = np.array(P_list, dtype=np.int64)
            # print(I_list, P_list)

                
            return video_data, I_list, P_list
    ...
    def __call__(self, sample_info):
        ...
        try:
            video_data, I_list, P_list = self.sparse_sampling_get_frameid_data(video_path, self.sequence_length, test_info)
        except:
...
def dali_pipeline(mode, source_params):
    ...
    if mode == "train":
        ...
        videos, I_lists, P_lists = fn.external_source(
        ...
        videos = videos.gpu()
        teacher_videos = videos.copy().gpu()
...
def dali_dataloader(
    ...
    output_map = ['videos', 'teacher_videos', 'I_lists', 'P_lists']
    if mode == "test":
